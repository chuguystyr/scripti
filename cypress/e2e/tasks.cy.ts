describe("testing tasks functionality", () => {
  ...
  it("should not add a new task when some fields are missing, (#TF1)", () => {
    cy.get("a").contains("New Task").click()
    cy.fixture("tasks").then(({ validTask }) => {
      cy.get('select[name="course"]').select(validTask.course)
  ...
  it("should not add a new task with invalid date, (#TF2)", () => {
    cy.get("a").contains("New Task").click()
    cy.fixture("tasks").then(({ validTask }) => {
  ...
  it("should add a new task when all fields are filled, (#TF3)", () => {
    cy.get("a").contains("New Task").click()
    cy.fixture("tasks").then(({ validTask }) => {
      cy.get('input[name="title"]').type(validTask.title)
      cy.get('select[name="course"]').select(validTask.course)
  ...
  it("should search for a task by title, (#TF4)", () => {
  ...
  it("should search for a task by title and don't show anything when no task with such title exists, (#TF5)", () => {
    cy.fixture("tasks").then(({ validTask: { title } }) => {
  ...
  it("should not add a new task with repeating title, (#TF9)", () => {
    cy.get("a").contains("New Task").click()
    cy.fixture("tasks").then(({ validTask }) => {
      cy.get('input[name="title"]').type(validTask.title)
      cy.get('select[name="course"]').select(validTask.course)
  ...
  it("should delete an existing task, (#TF10)", () => {
    cy.fixture("tasks").then(({ validTask: { title } }) => {
