...
export const setSchedule = async (prevState: any, form:FormData) => {
  const formData = Object.fromEntries(form.entries()) as {[key: string]: string};
  if (
    formData['from'] !== '' &&
    formData['to'] !== '' &&
    ...
    try {
      const result = await User.findOneAndUpdate({_id: userId.id}, {$push: {schedules: schedule}}, {new: true});
      if (result) {
